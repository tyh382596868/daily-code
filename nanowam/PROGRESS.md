...
- [ ] User: `pip install` 依赖(`torch transformers accelerate einops opencv-python pyyaml`,**diffusers 先不装,由 00 决定**)
- [ ] User: 从 HF 下载 `Wan-AI/Wan2.1-T2V-1.3B`,记本地路径填进 `nanowam/configs/wan21_1_3B.yaml`
- [ ] User: 先跑 `nanowam/stage0_sanity/00_env_check.py`,**把 stdout 贴回 chat**。Claude 据此决定 01/02/03 走 diffusers 路径(A)还是 wan 官方路径(B)
