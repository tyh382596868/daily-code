...
file: src/lerobot/policies/normalize.py
permalink: https://github.com/huggingface/lerobot/blob/main/src/lerobot/policies/normalize.py
difficulty: intermediate
read_time: ~10 min
tags: [code-of-the-day, vla]
---

# LeRobot Normalize / Unnormalize：把机器人数据变成策略友好的张量 / LeRobot Normalize / Unnormalize: Turn Robot Data into Policy-Friendly Tensors

> **一句话 / In one line**: LeRobot 把归一化做成 `nn.Module`，训练前按 feature 逐项变换，推理后再反变换动作，让策略只学习尺度稳定的空间。 / LeRobot implements normalization as `nn.Module`s: transform each feature before training, then invert actions after inference so the policy learns in a stable scale.
...
Source / 来源：[`huggingface/lerobot` `src/lerobot/policies/normalize.py`](https://github.com/huggingface/lerobot/blob/main/src/lerobot/policies/normalize.py)
...
- Repository source: [huggingface/lerobot](https://github.com/huggingface/lerobot/blob/main/src/lerobot/policies/normalize.py)
