...
read_time: ~10 min
tags: [code-of-the-day, robotics]
---

# OpenPI Transforms：用 dataclass 组织可组合数据变换 / OpenPI Transforms: Composable Data Transforms with Dataclasses

> **一句话 / In one line**: OpenPI 的数据管道把每个变换做成可调用对象，输入输出都是字典式样本，因此机器人状态、图像、语言和动作可以按阶段逐步标准化。 / OpenPI represents each data transform as a callable object over dict-like samples, so state, image, language, and action fields can be normalized stage by stage.
...
Source / 来源：[`Physical-Intelligence/openpi` `src/openpi/transforms.py`](https://github.com/Physical-Intelligence/openpi/blob/main/src/openpi/transforms.py)
...
- Repository source: [Physical-Intelligence/openpi](https://github.com/Physical-Intelligence/openpi/blob/main/src/openpi/transforms.py)
