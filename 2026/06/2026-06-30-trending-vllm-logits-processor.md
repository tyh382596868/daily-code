...
permalink: https://github.com/vllm-project/vllm/blob/main/vllm/logits_process.py
difficulty: intermediate
read_time: ~10 min
tags: [code-of-the-day, infrastructure]
---

# vLLM LogitsProcessor：采样前最后一公里的 logits 改写 / vLLM `LogitsProcessor`: The Last-Mile Logit Rewrite Before Sampling

> **一句话 / In one line**: vLLM 在采样前集中处理 logits：温度、惩罚、top-k/top-p 等策略都在同一条路径里改写分布，然后再交给 sampler。 / vLLM centralizes logit rewriting before sampling: temperature, penalties, top-k/top-p, and related policies shape the distribution before the sampler draws tokens.
...
Source / 来源：[`vllm-project/vllm` `vllm/logits_process.py`](https://github.com/vllm-project/vllm/blob/main/vllm/logits_process.py)
...
- Repository source: [vllm-project/vllm](https://github.com/vllm-project/vllm/blob/main/vllm/logits_process.py)
